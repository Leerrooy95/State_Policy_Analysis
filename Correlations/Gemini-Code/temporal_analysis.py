...
Input files:
  ...
  - 02_CSVs_and_Datasets/Multi_State/Federal_Funding_Withholding_2025-2026.csv
  - 02_CSVs_and_Datasets/Copilot_Agent/state_posture_lookup.csv

Output:
  - Correlations/Gemini/temporal_analysis_final.csv
  - Correlations/Gemini/temporal_summary.md
"""
...
GEMINI_DIR = os.path.join(REPO_ROOT, "Correlations", "Gemini")
DATASETS_DIR = os.path.join(REPO_ROOT, "02_CSVs_and_Datasets")
COPILOT_DIR = os.path.join(DATASETS_DIR, "Copilot_Agent")
MULTI_STATE_DIR = os.path.join(DATASETS_DIR, "Multi_State")
...
legislation = leg_expanded.drop_duplicates(
    subset=["state_code", "bill"], keep="first"
)
print(
    f"Loaded legislation: {len(legislation)} unique bills"
)

# Federal funding tracker
funding = pd.read_csv(
    os.path.join(MULTI_STATE_DIR, "Federal_Funding_Withholding_2025-2026.csv")
)
print(f"Loaded federal funding: {len(funding)} actions")

# 27-state posture lookup (includes Neutral/Mixed)
posture = pd.read_csv(os.path.join(COPILOT_DIR, "expanded_state_posture_lookup.csv"))
...
csv_path = os.path.join(GEMINI_DIR, "temporal_analysis_final.csv")
...
summary_lines.append("")
summary_lines.append(
    "*Analysis generated by `temporal_analysis.py`. "
    "Data sources: 02_CSVs_and_Datasets/ and Correlations/ folders.*"
)
summary_lines.append("")
...
# ---------------------------------------------------------------------------
md_path = os.path.join(GEMINI_DIR, "temporal_summary.md")
